...
public:
	NVMeCommand(Device *dev)
		: mpEngine(nullptr), mXferMode(DataXfer_PRP), mpatMngr(nullptr), patSwitch(false), mbufMngr(nullptr), mpdev(dev)
	{
...
public:
	...
	void SetFusedFlag(void)
	{
		mbFusedCmd = true;
		mFusedMode = Fused_None;
	};
	void ClrFusedFlag(void)
	{
		mbFusedCmd = false;
		mFusedMode = Fused_None;
	};
