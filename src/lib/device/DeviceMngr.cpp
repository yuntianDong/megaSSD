...
DeviceMngr::DeviceMngr(const char *bdf, bool ifrestore) : mpPcieDeviceHandler{nullptr}, mpNVMeDeviceHandler{nullptr}, mbdf{nullptr}
{
    ...
    while (i < MAX_DEVICE_NUM && mbdf[i] != nullptr)
    {
        ...
        if (driverInUse = pci_get_string_property(mppcidev, PCI_FILL_DRIVER))
        {
            ...
            {
                mpNVMeDeviceHandler[i] = (DeviceHandler *)new SPDKHandler(mbdf[i], i, ifrestore);
            }
            else
            {
...
int DeviceMngr::GetDevAllBdf(const char *bdf, const char *mbdf[])
{
    ...
    */
    ...
    for (int idx = 0; idx < mDevScan->GetDevNodeCount(); idx++)
    {
        ...
        if (mdsn != (uint64_t)-1 && dsn != (uint64_t)-1 && mdsn == dsn)
        {
            mbdf[++bdfIdx] = scanbdf;
        }
    }
