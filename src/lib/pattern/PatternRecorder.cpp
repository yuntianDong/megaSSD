...
bool PatRecBitmap::Merge(Buffers &buf, uint32_t bufOffset, uint64_t offset, uint16_t length)
{
		...
		((uint64_t)muBufSize) * 2 < (offset + length))
	{
		...
		LOGDEBUG("bitmapBufSize:%lu,bufSize:%lu,bufOffset:%u,bufSectorSize:%u,length:%u,offset:%lu", ((uint64_t)muBufSize) * 2, buf.GetBufSize(), bufOffset, buf.GetSectorSize(), length, offset);
		return false;
	}
